...
class PostListViewTests(APITestCase):
    ...
    def test_logged_in_user_can_create_post(self):
        ...
        response = self.client.post(
            self.url, {'title': 'a title', 'content': 'some content'}
        )
...
class PostDetailViewTests(APITestCase):
    def setUp(self):
        ...
        self.brian = User.objects.create_user(username='brian',
                                              password='pass')
        self.adam_post = Post.objects.create(
            owner=self.adam, title='a title', content='adams content'
        )
        self.brian_post = Post.objects.create(
            owner=self.brian, title='another title', content='brians content'
        )
        self.detail_url_adam = reverse(
            'post-detail', kwargs={'pk': self.adam_post.pk}
        )
        self.detail_url_brian = reverse(
            'post-detail', kwargs={'pk': self.brian_post.pk}
        )
    ...
    def test_user_can_update_own_post(self):
        ...
        response = self.client.put(
            self.detail_url_adam,
            {'title': 'a new title', 'content': 'updated content'}
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
