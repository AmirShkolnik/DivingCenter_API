    ...
    def create(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            try:
                ...
                logger.warning(f"Booking creation failed: Duplicate booking attempt")
                error_message = "A booking for this course, date, and time already exists. Please choose a different date or time."
    ...
    def update(self, request, *args, **kwargs):
        ...
        if serializer.is_valid():
            try:
                ...
                logger.info(f"Booking updated successfully for user {request.user.username}")
                return Response(serializer.data)
            except IntegrityError:
                logger.warning(f"Booking update failed: Duplicate booking attempt")
                error_message = "A booking for this course, date, and time already exists. Please choose a different date or time."
                return Response({"non_field_errors": [error_message]}, status=status.HTTP_400_BAD_REQUEST)
        else:
